      ...
      <div style={{ display: 'flex' }}>
        <div style={{ flexGrow: 1 }}>
          <Dropdown
            ...
            onSelect={(option) =>
              this.setState({ useSameTransition: option.id })
            }
        ...
        >
          <Animator
            childClassName={css.absolute}
            translate={{
              enter: { direction: directionIn, size: '100%' },
              exit: { direction: directionOut, size: '100%' },
              useSameTransition: this.state.useSameTransition,
            }}
          >
            {show && (
              <StyledDiv style={{ opacity: '0.5' }}>
                I am transparent so you can see the animation is smooth when
                using
            ...
            {!show && (
              <OrangeDiv style={{ opacity: '0.5' }}>
                Check out the code to see how it is being done
              </OrangeDiv>
            )}
          </Animator>
        </div>
