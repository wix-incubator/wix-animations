...
const createSizeArray = (size) => {
  return new Array(size + 1)
    .fill(0)
    .map((element, id) => ({ id: id * 10, value: id * 10 }));
};

const MockDiv = ({ children }) => {
  return (
    <div className={css.shukiInner} style={{ overflow: 'hidden' }}>
      {children}
    </div>
  );
};
...
class AnimatedExample extends React.Component {
    ...
    return (
      <div>
        <ToggleSwitch
          ...
          onChange={() => this.setState({ [option]: !this.state[option] })}
        />
        {option}
      </div>
    );
  ...
  getSequenceValue() {
    ...
    return this.state.sequenceOption !== 'default'
      ? this.state.sequenceOption
      : true;
  }
  ...
  buildTranslateString() {
    ...
    const size =
      translateSizeIn === translateSizeOut
        ? `{size: ${translateSizeIn}}`
        : `{size: {in: ${translateSizeIn}, out: ${translateSizeOut}`;
  ...
  buildTranslateObject() {
    ...
    return {
      ...
      size:
        translateSizeIn === translateSizeOut
          ? translateSizeIn
          : { in: translateSizeIn, out: translateSizeOut },
    };
  }
    ...
    return (
      <div style={{ height: '250px' }}>
        <Container>
          <Row>
            {' '}
            <Col span="">Animation component </Col>
          </Row>
          <Row>
            <Col span="4">
              <Row>
              ...
              <Row>{this.myToggle('opacity')}</Row>
              <Row>{this.myToggle('scale')}</Row>
              <Row>{this.myToggle('height')}</Row>
              <Row>{this.myToggle('width')}</Row>
              <Row>{this.myToggle('translate')}</Row>
              {this.state.translate && (
                <Row>
                  <Col span="12">Translate Options</Col>
                  <Col span="6">
                    Size In
                    <Dropdown
                      selectedId={this.state.translateSizeIn}
                      onSelect={(option) =>
                        this.setState({ translateSizeIn: option.id })
                      }
                    ...
                    <Dropdown
                      ...
                      onSelect={(option) =>
                        this.setState({ translateSizeOut: option.id })
                      }
                      options={this.sizeOptions}
                    />
                  </Col>
                  <Col span="6">
                    ...
                    <Dropdown
                      ...
                      onSelect={(option) =>
                        this.setState({ direction: option.id })
                      }
              ...
              <Row>
                ...
                {this.state.sequence && (
                  <Dropdown
                    ...
                    onSelect={(option) =>
                      this.setState({ sequenceOption: option.id })
                    }
            ...
            <Col span="8">
              <pre>
                ...
                {this.state.sequence && this.state.sequenceOption !== 'default'
                  ? `="${this.state.sequenceOption}"`
                  : ''}
                {this.state.debug !== 'none'
              ...
              <Animator
                ...
                translate={
                  this.state.translate ? this.buildTranslateObject() : false
                }
                sequence={this.getSequenceValue()}
                timing={
                  this.state.timing === 'large' ? false : this.state.timing
                }
              >
                {this.state.show && (
                  <MockDiv
                    animatorChildStyle={{ flexGrow: 2 }}
                    animatorChildClassName={css.shukiChild}
                  >
          ...
          <Row>
            <Col span={12}>
              Rules: 1. Translate - The height and width belongs to the parent
              and not the child so it translates according to them
            </Col>
          </Row>
        </Container>
      </div>
