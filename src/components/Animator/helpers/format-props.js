...
import { timings, sequences, propsDefault } from '../constants/constants';

const getDataOrDefault = (arr, value, name) => {
  return arr.indexOf(value) > -1 ? value : propsDefault[name];
};

const getScale = (scale, timing) =>
  ['small', 'medium'].indexOf(timing) > -1 ? 0.75 : 0.9;

const getDelay = (value) => {
  const delay =
    typeof value === 'object' ? value : { enter: value, exit: value };
...
const getData = (props) =>
  Object.keys(propsMap).reduce((data, propName) => {
    return {
      [propName]: getPropData(propName, props[propName], props),
      ...data,
    };
  }, {});
